    ...
    stages {
        ...
        stage('Docker Build') {
            steps {
                ...
                docker build --no-cache . --label build_date=$(date +%Y-%m-%d) -t $(echo $DOCKERUSER)/ytdl:$(cat version)
                docker push $(echo $DOCKERUSER)/ytdl:$(cat version)
                docker build --no-cache . --label build_date=$(date +%Y-%m-%d) -t $(echo $DOCKERUSER)/ytdl:latest
